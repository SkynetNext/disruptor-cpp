...
public:
  ...
  T& get(int64_t sequence) override {
    return (*data_)[static_cast<size_t>(sequence)];
  }
...
TEST(EventPollerTest, shouldPollForEvents) {
  ...
  public:
    ...
    void*& get(int64_t sequence) override {
      return (*d_)[static_cast<size_t>(sequence)];
    }
  ...
  struct Handler final : public PollerT::Handler {
    bool onEvent(void*& /*event*/, int64_t /*sequence*/, bool /*endOfBatch*/) override {
      return false;
    }
  } handler;
