...
template <typename, ProducerType, typename>
class Disruptor;
...
public:
  ...
  and_(EventHandlerGroup<T, Producer, WaitStrategyT>& otherHandlerGroup) {
    ...
    return EventHandlerGroup<T, Producer, WaitStrategyT>(
      *disruptor_, *consumerRepository_, combined.data(), static_cast<int>(combined.size()));
  }

  EventHandlerGroup<T, Producer, WaitStrategyT> and_(EventProcessor* const* processors, int count) {
    std::vector<Sequence*> combined;
  ...
  EventHandlerGroup<T, Producer, WaitStrategyT> handleEventsWith(Handlers&... handlers) {
    return disruptor_->createEventProcessors(sequences_.data(), static_cast<int>(sequences_.size()),
                                             handlers...);
  }

  template <typename... Factories>
  EventHandlerGroup<T, Producer, WaitStrategyT> thenFactories(Factories&... factories) {
    return handleEventsWithFactories(factories...);
  }

  template <typename... Factories>
  EventHandlerGroup<T, Producer, WaitStrategyT> handleEventsWithFactories(Factories&... factories) {
    return disruptor_->createEventProcessors(sequences_.data(), static_cast<int>(sequences_.size()),
                                             factories...);
  }

  BarrierPtr asSequenceBarrier() {
    auto barrier = disruptor_->getRingBuffer().newBarrier(sequences_.data(),
                                                          static_cast<int>(sequences_.size()));
