...
// Source:
// reference/disruptor/src/perftest/java/com/lmax/disruptor/sequenced/OneToOneSequencedThroughputTest.java
//
...
public:
  ...
  using RingBufferType =
    disruptor::RingBuffer<ValueEvent,
                          disruptor::SingleProducerSequencer<disruptor::YieldingWaitStrategy>>;
  using BarrierType = disruptor::ProcessingSequenceBarrier<
    disruptor::SingleProducerSequencer<disruptor::YieldingWaitStrategy>,
  ...
  OneToOneSequencedThroughputTest()
    ...
    , ws_()
    , ringBuffer_(disruptor::SingleProducerRingBuffer<ValueEvent, disruptor::YieldingWaitStrategy>::
                    createSingleProducer(ValueEvent::EVENT_FACTORY, kBufferSize, ws_))
  ...
  int getRequiredProcessorCount() const {
    return 2;
  }
  ...
  PerfTestContext runDisruptorPass() {
    ...
    // Producer loop
    // Java: for (long i = 0; i < ITERATIONS; i++) { long next = rb.next();
    // rb.get(next).setValue(i); rb.publish(next); }
    ...
    // Java: perfTestContext.setDisruptorOps((ITERATIONS * 1000L) / (System.currentTimeMillis() -
    // start)); System.currentTimeMillis() returns milliseconds, so we need to match that
  ...
  void testImplementations() {
    ...
    for (int i = 0; i < kRuns; i++) {
      ...
      // Java: System.out.format("Run %d, Disruptor=%,d ops/sec BatchPercent=%.2f%%
      // AverageBatchSize=%,d\n", ...)
...
void PerfTest_OneToOneSequencedThroughputTest(benchmark::State& state) {
  ...
  if (!test) {
    ...
    if (test->getRequiredProcessorCount() > availableProcessors) {
      std::print(
        "*** Warning ***: your system has insufficient processors to execute the test efficiently. ");
      std::print("Processors required = {} available = {}\n", test->getRequiredProcessorCount(),
                 availableProcessors);
    }
    if (!headerPrinted) {
      std::print("Starting Disruptor tests\n");
  ...
  for (auto _ : state) {
    ...
    // Print result in Java's format: "Run %d, Disruptor=%,d ops/sec BatchPercent=%.2f%%
    // AverageBatchSize=%,d\n"
    int64_t ops = context.getDisruptorOps();
    double batchPercent = context.getBatchPercent() * 100.0;
    double avgBatchSize = context.getAverageBatchSize();
    std::print("Run {}, Disruptor={} ops/sec BatchPercent={:.2f}% AverageBatchSize={:.0f}\n",
               runCounter++, ops, batchPercent, avgBatchSize);

    // Report to Google Benchmark for statistics
    state.counters["ops_per_sec"] =
      benchmark::Counter(context.getDisruptorOps(), benchmark::Counter::kIsRate);
    state.counters["batch_percent"] = benchmark::Counter(context.getBatchPercent() * 100.0);
    state.counters["avg_batch_size"] = benchmark::Counter(context.getAverageBatchSize());
  }
