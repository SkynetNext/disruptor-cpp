...
struct DataEvent {
  ...
  static std::shared_ptr<disruptor::EventFactory<DataEvent<T>>> factory() {
    struct F final : public disruptor::EventFactory<DataEvent<T>> {
      DataEvent<T> newInstance() override {
        return DataEvent<T>();
      }
    };
...
template <typename T, typename PollerT>
static T getNextValue(PollerT& poller) {
  T out{};
...
int main() {
  ...
  using Seq = disruptor::MultiProducerSequencer<disruptor::BlockingWaitStrategy>;
  using RB = disruptor::RingBuffer<DataEvent<void*>, Seq>;
  auto ringBuffer = RB::createMultiProducer(DataEvent<void*>::factory(), 1024, ws);
